...
# Execute command
ls -la output/ 2>/dev/null || echo "Output directory does not exist yet"

# Execute command
python3 -c "
import mammoth
result = mammoth.extract_raw_text({'path': 'input/Tutorial.docx'})
print(result.value)
" 2>&1 | head -200

# Execute command
NODE_PATH=/.aistudio_shared/node-v24.13.0/lib/node_modules node -e "
const mammoth = require('mammoth');

mammoth.extractRawText({path: 'input/Tutorial.docx'})
...
doc = Document('input/Tutorial.docx')

# Try to get TOC content - look at first 100 paragraphs
for i, para in enumerate(doc.paragraphs[:100]):
    if para.text.strip():
        style_name = para.style.name if para.style else 'No style'
        print(f'{i}: [{style_name}] {para.text[:150]}')
" 2>&1

# Execute command
python -m markitdown input/Tutorial.docx 2>&1 | head -200

# Execute command
python -m markitdown input/sample-word-document.docx 2>&1

# Execute command
python -m markitdown input/sample.docx 2>&1

# Execute command
python -m markitdown input/template.docx 2>&1
...
# Create /root/extract_text.js
mkdir -p $(dirname /root/extract_text.js)
cat << 'HEREDOC_EOF' > /root/extract_text.js
const mammoth = require("mammoth");
...
# Execute command
NODE_PATH=/.aistudio_shared/node-v24.13.0/lib/node_modules node extract_text.js /root/Tutorial.docx > /tmp/tutorial_raw.txt 2>&1 && echo "Tutorial extraction done, size: $(wc -c < /tmp/tutorial_raw.txt) bytes"

# Execute command
NODE_PATH=/.aistudio_shared/node-v24.13.0/lib/node_modules node /root/extract_text.js /root/Tutorial.docx 2>&1 | head -50

# Execute command
npm install mammoth 2>&1 | tail -10

# Execute command
node /root/extract_text.js /root/Tutorial.docx > /tmp/tutorial_raw.txt 2>&1 && echo "Tutorial extraction done, size: $(wc -c < /tmp/tutorial_raw.txt) bytes"

# Execute command
node /root/extract_text.js /root/sample-word-document.docx > /tmp/sample-word-document.docx.txt 2>&1 && echo "sample-word-document.docx done"

# Execute command
node /root/extract_text.js /root/sample.docx > /tmp/sample.docx.txt 2>&1 && echo "sample.docx done"

# Execute command
node /root/extract_text.js /root/template.docx > /tmp/template.docx.txt 2>&1 && echo "template.docx done"

# Create /root/create_json.py
mkdir -p $(dirname /root/create_json.py)
cat << 'HEREDOC_EOF' > /root/create_json.py
import json
import pathlib

# The 14 TOC section titles from Tutorial.docx (extracted from the raw text)
# These are taken from lines 31-57 of the mammoth output, cleaned of page numbers
sections = [
    "The basics",
    "The Write method",
    "Conditional content",
...
# Compute keyword check for other files
def has_keyword(filepath):
    """Check if file contains 'sharpdocx' (case-insensitive)"""
    text = pathlib.Path(filepath).read_text(errors="ignore")
    return "sharpdocx" in text.lower()

# Check each file
checks = [
    ("sample-word-document.docx", has_keyword("/tmp/sample-word-document.docx.txt")),
    ("sample.docx", has_keyword("/tmp/sample.docx.txt")),
    ("template.docx", has_keyword("/tmp/template.docx.txt")),
]
...
output = {
    ...
    "other_files_keyword_check": [
        {"file": fname, "contains_sharpdocx_keyword": kw_found}
        for fname, kw_found in checks
    ]
}

# Validate
assert output["tutorial_toc"]["section_count"] == 14
assert len(output["tutorial_toc"]["sections"]) == 14
assert [x["file"] for x in output["other_files_keyword_check"]] == [
    "sample-word-document.docx", "sample.docx", "template.docx"
]

# Create output directory and write JSON
output_dir = pathlib.Path("/root/output")
output_dir.mkdir(parents=True, exist_ok=True)
output_path = output_dir / "sharpdocx_toc_and_keyword_check.json"
output_path.write_text(json.dumps(output, ensure_ascii=False, indent=2))

print(f"JSON written to: {output_path}")
print(f"Section count: {output['tutorial_toc']['section_count']}")
print(f"Sections: {output['tutorial_toc']['sections']}")
print(f"Keyword checks: {output['other_files_keyword_check']}")

HEREDOC_EOF

# Execute command
python3 /root/create_json.py

# Execute command
python3 -c '
import json
p="/root/sharpdocx_toc_and_keyword_check.json"
d=json.load(open(p))
assert d["tutorial_toc"]["section_count"]==14, "section_count must be 14"
assert len(d["tutorial_toc"]["sections"])==14, "sections length must be 14"
assert [x["file"] for x in d["other_files_keyword_check"]] == [
    "sample-word-document.docx","sample.docx","template.docx"
], "file ordering must match"
print("All validations passed!")
print(f"- section_count: {d[\"tutorial_toc\"][\"section_count\"]}")
print(f"- sections length: {len(d[\"tutorial_toc\"][\"sections\"])}")
print(f"- file order: {[x[\"file\"] for x in d[\"other_files_keyword_check\"]]}")
'
...
print("All validations passed!")
toc = d["tutorial_toc"]
print(f"- section_count: {toc['section_count']}")
print(f"- sections length: {len(toc['sections'])}")
files_order = [x["file"] for x in d["other_files_keyword_check"]]
print(f"- file order: {files_order}")
EOF
