...
# Read all three DICOM files
ct = pydicom.dcmread('input/CT_small.dcm')
mr = pydicom.dcmread('input/MR_small.dcm')
plan = pydicom.dcmread('input/rtplan.dcm')

def get_field(ds, field):
    return str(getattr(ds, field, '')) if hasattr(ds, field) else ''

fields = ['PatientName', 'PatientID', 'PatientSex', 'StudyID', 'StudyInstanceUID']

print('=== CT Fields ===')
for f in fields:
    print(f'{f}: {repr(get_field(ct, f))}')

print('\n=== MR Fields ===')
for f in fields:
    print(f'{f}: {repr(get_field(mr, f))}')

print('\n=== RT Plan Fields ===')
for f in fields:
    print(f'{f}: {repr(get_field(plan, f))}')
"


# Create /root/harbor_workspaces/task_T361_run2/expectation_tests.py
mkdir -p $(dirname /root/harbor_workspaces/task_T361_run2/expectation_tests.py)
cat << 'HEREDOC_EOF' > /root/harbor_workspaces/task_T361_run2/expectation_tests.py
...
plan_path = "/root/rtplan.dcm"
out_path = "/root/dicom_plan_match.json"

# Read all three DICOM files
ct_ds = pydicom.dcmread(ct_path)
mr_ds = pydicom.dcmread(mr_path)
plan_ds = pydicom.dcmread(plan_path)
...
ct = {k: get_str(ct_ds, k) for k in fields}
mr = {k: get_str(mr_ds, k) for k in fields}
plan = {k: get_str(plan_ds, k) for k in fields}

# Print extracted values for verification
print("CT fields:", ct)
print("MR fields:", mr)
print("Plan fields:", plan)

# Compute match scores
def score(img, plan):
    s = 0
    s += 3 if img["PatientID"] == plan["PatientID"] else 0
    s += 2 if img["PatientName"] == plan["PatientName"] else 0
    s += 1 if img["PatientSex"] == plan["PatientSex"] else 0
    s += 1 if img["StudyID"] == plan["StudyID"] else 0
    s += 1 if img["StudyInstanceUID"] == plan["StudyInstanceUID"] else 0
    return s

score_ct = score(ct, plan)
score_mr = score(mr, plan)

print(f"\nScore CT vs Plan: {score_ct}")
print(f"Score MR vs Plan: {score_mr}")

# Determine best match
if score_ct > score_mr:
    best = "CT"
elif score_mr > score_ct:
    best = "MR"
else:
    best = "TIE"

print(f"Best match modality: {best}")

# Create output directory if needed
os.makedirs(os.path.dirname(out_path), exist_ok=True)

# Build the JSON payload with exact schema
payload = {
    "plan_patient_id": plan["PatientID"],
    "plan_patient_name": plan["PatientName"],
    "ct_patient_id": ct["PatientID"],
    "ct_patient_name": ct["PatientName"],
    "mr_patient_id": mr["PatientID"],
    "mr_patient_name": mr["PatientName"],
    "score_ct_vs_plan": score_ct,
    "score_mr_vs_plan": score_mr,
    "best_match_modality": best
}
