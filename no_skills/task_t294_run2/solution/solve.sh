#!/bin/bash
# solve.sh - Recreates output files for the highest cancellation airline task

set -e

# HARBOR_PATH_FIX_START
BASE_FIX="/root/harbor_workspaces/task_T294_run2"
mkdir -p "$BASE_FIX/input" "$BASE_FIX/output"
if [ ! -e "$BASE_FIX/input/flights_2.csv" ] && [ -e "/root/flights_2.csv" ]; then cp -f "/root/flights_2.csv" "$BASE_FIX/input/flights_2.csv"; fi
if [ ! -e "$BASE_FIX/input/airlines.csv" ] && [ -e "/root/airlines.csv" ]; then cp -f "/root/airlines.csv" "$BASE_FIX/input/airlines.csv"; fi
# HARBOR_PATH_FIX_END

# Create output directory
mkdir -p /root/harbor_workspaces/task_T294_run2/output

# Create analyze_cancellations.py
cat > /root/harbor_workspaces/task_T294_run2/analyze_cancellations.py << 'PYTHON_SCRIPT'
import pandas as pd
import json

# Paths
flights_path = "/root/harbor_workspaces/task_T294_run2/input/flights_2.csv"
airlines_path = "/root/harbor_workspaces/task_T294_run2/input/airlines.csv"
output_path = "/root/harbor_workspaces/task_T294_run2/output/highest_cancellation_airline.json"

# Step 1: Load CSVs with proper dtype handling
# Force arr_delay to string so "NA" is preserved as literal string (not converted to NaN)
flights = pd.read_csv(
    flights_path,
    dtype={"arr_delay": "string", "carrier": "string"},
    keep_default_na=False  # Prevent "NA" from being converted to NaN
)
...
# Verify "NA" exists in arr_delay
print(f"Total flights: {len(flights)}")
print(f"Flights with arr_delay='NA': {(flights['arr_delay'] == 'NA').sum()}")
print(f"Unique carriers: {flights['carrier'].unique()}")

# Step 2: Compute totals and cancelled counts per carrier
flights["is_cancelled"] = flights["arr_delay"].eq("NA")

stats = flights.groupby("carrier").agg(
    total_flights=("carrier", "count"),
    cancelled_flights=("is_cancelled", "sum"),
).reset_index()

stats["cancellation_rate"] = stats["cancelled_flights"] / stats["total_flights"]

print("\nCancellation stats per carrier:")
print(stats.to_string())

# Step 3: Apply tie-break rules deterministically
# Sort by: highest rate, then most flights, then lexicographically smallest carrier
sorted_stats = stats.sort_values(
    by=["cancellation_rate", "total_flights", "carrier"],
    ascending=[False, False, True]
)

print("\nSorted stats (winner at top):")
print(sorted_stats.to_string())

winner = sorted_stats.iloc[0]
print(f"\nWinner carrier: {winner['carrier']}")

# Step 4: Join winner to airlines.csv to get airline name
winner_df = pd.DataFrame([winner])
joined = winner_df.merge(airlines, left_on="carrier", right_on="Code", how="left")

airline_name = joined.loc[0, "Description"]
print(f"Airline name: {airline_name}")

# Step 5: Write JSON output with correct numeric types
output = {
    "carrier": str(winner["carrier"]),
    "airline_name": str(airline_name),
    "total_flights": int(winner["total_flights"]),
    "cancelled_flights": int(winner["cancelled_flights"]),
    "cancellation_rate": float(winner["cancellation_rate"]),
}
...
# Run the analysis script to generate the output JSON
python3 /root/harbor_workspaces/task_T294_run2/analyze_cancellations.py

# Create expectation_tests.py
cat > /root/harbor_workspaces/task_T294_run2/expectation_tests.py << 'PYTHON_TESTS'
...
# Constants
OUTPUT_FILE = "/root/harbor_workspaces/task_T294_run2/output/highest_cancellation_airline.json"
OUTPUT_DIR = "/root/harbor_workspaces/task_T294_run2/output"
FLIGHTS_FILE = "/root/harbor_workspaces/task_T294_run2/input/flights_2.csv"
AIRLINES_FILE = "/root/harbor_workspaces/task_T294_run2/input/airlines.csv"
...
# Verify the syntax of the test file
python3 -m py_compile /root/harbor_workspaces/task_T294_run2/expectation_tests.py && echo "expectation_tests.py syntax OK"

echo "solve.sh completed successfully"

# HARBOR_OUTPUT_FIX_START
if [ -f "/root/harbor_workspaces/task_T294_run2/output/highest_cancellation_airline.json" ]; then cp -f "/root/harbor_workspaces/task_T294_run2/output/highest_cancellation_airline.json" "/root/highest_cancellation_airline.json"; fi
