...
if [ -f /root/reconcile.py ]; then
  ...
  awk '{ if ($0 == "set -e") { print "set +e"; next } print }' /solution/solve.sh > /tmp/solve_relaxed.sh
  bash /tmp/solve_relaxed.sh || true
fi
_expected="/root/sio2_mp6930_verification.xlsx"
